                ...
                <div class="flex items-center justify-between">
                    ...
                    <div class="flex space-x-3">
                        ...
                        <button wire:click="deleteProduct"
                                wire:confirm="{{ __('app.delete_product_confirm') }}"
            ...
            <div class="aspect-square bg-gradient-to-br from-gray-100 to-gray-200 dark:from-gray-700 dark:to-gray-800 flex items-center justify-center relative">
                ...
                @else
                    <div class="text-center">
                        ...
                        <p class="text-gray-500 dark:text-gray-400 text-lg">{{ __('app.no_image_available') }}</p>
                    </div>
                @endif
            </div>
                ...
                <div class="space-y-8">
                    ...
                    <div>
                        ...
                        <div class="flex items-center space-x-4 text-sm text-gray-500 dark:text-gray-400">
                            <span class="inline-flex items-center">
                                ...
                                {{ __('app.created') }} {{ $product->created_at->diffForHumans() }}
                            </span>
                            <span class="inline-flex items-center">
                                <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"></path>
                                </svg>
                                {{ __('app.updated') }} {{ $product->updated_at->diffForHumans() }}
                            </span>
                        </div>
                    </div>
                    ...
                    @if($product->description)
                        <div class="bg-gray-50 dark:bg-gray-700/50 rounded-xl p-6">
                            <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-3">{{ __('app.description') }}</h3>
                    ...
                    <div class="grid grid-cols-1 gap-6">
                        <div class="bg-gray-50 dark:bg-gray-700/50 rounded-xl p-6">
                            <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">{{ __('app.product_information') }}</h3>
                            <dl class="space-y-4">
                                <div class="flex justify-between">
                                    <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">{{ __('app.product_name') }}</dt>
                                    <dd class="text-sm text-gray-900 dark:text-white font-medium">{{ $product->name }}</dd>
                                </div>
                                <div class="flex justify-between">
                                    <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">{{ __('app.price') }}</dt>
                                    <dd class="text-sm text-gray-900 dark:text-white font-semibold">{{ $product->formatted_price }}</dd>
                                </div>
                                <div class="flex justify-between">
                                    <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">{{ __('app.created_date') }}</dt>
                                    <dd class="text-sm text-gray-900 dark:text-white">{{ $product->created_at->format('F j, Y \a\t g:i A') }}</dd>
                                </div>
                                <div class="flex justify-between">
                                    <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">{{ __('app.last_updated') }}</dt>
                                    <dd class="text-sm text-gray-900 dark:text-white">{{ $product->updated_at->format('F j, Y \a\t g:i A') }}</dd>
                                </div>
                                <div class="flex justify-between">
                                    <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">{{ __('app.product_id') }}</dt>
