<div class="py-8" wire:key="order-list-{{ now()->timestamp }}">
        ...
        <div class="space-y-8">
            ...
            <div class="bg-gradient-to-r from-purple-600 to-blue-700 rounded-2xl p-8">
                <div class="flex flex-col md:flex-row md:items-center md:justify-between">
                    <div>
                        <h1 class="text-3xl font-bold mb-2">{{ __('app.order_history') }}</h1>
                        <p class="text-purple-100 text-lg">{{ __('View and manage all orders') }}</p>
                    </div>
                    <div class="mt-6 md:mt-0">
                        <div class="bg-white/20 backdrop-blur-sm rounded-lg p-4">
                            <div class="text-2xl font-bold">{{ $orders->total() }}</div>
                            <div class="text-purple-100">{{ __('Total Orders') }}</div>
                        </div>
                    </div>
                </div>
            </div>
                        ...
                        <div class="bg-gradient-to-r from-purple-50 to-blue-50 dark:from-purple-900/20 dark:to-blue-900/20 p-4 border-b border-gray-200 dark:border-gray-700">
                            <div class="flex items-center justify-between">
                                ...
                                <div class="text-right">
                                    ...
                                    <div class="text-xs text-gray-500 dark:text-gray-400">{{ $order->total_items }} {{ __('app.items') }}</div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="p-4">
                            <div class="space-y-3">
                                ...
                                <div class="flex items-center space-x-2">
                                    ...
                                    <span class="text-sm text-gray-600 dark:text-gray-400">{{ __('app.cashier_name') }}: {{ $order->cashier_name }}</span>
                                </div>

                                <!-- Order Items Preview -->
                                <div>
                                    <p class="text-sm font-medium text-gray-900 dark:text-white mb-2">{{ __('app.items') }}:</p>
                                    <div class="space-y-1">
                                        ...
                                        @if($order->orderItems->count() > 3)
                                            <div class="text-xs text-gray-500 dark:text-gray-400">
                                                +{{ $order->orderItems->count() - 3 }} {{ __('app.more_items') }}
                                            </div>
                                        @endif
                                    </div>
                                </div>
                        ...
                        <div class="bg-gray-50 dark:bg-gray-700/50 px-4 py-3 flex items-center justify-between">
                            ...
                            <div class="flex items-center space-x-2">
                                ...
                                <button 
                                    ...
                                    class="inline-flex items-center px-2 py-1 bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-300 text-xs font-medium rounded-md hover:bg-red-200 dark:hover:bg-red-900/50 transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
                                    title="{{ __('app.delete_order') }}">
                                    <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        @else
            ...
            <div class="text-center py-16">
                <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 p-12">
                    ...
                    @if($search)
                        <h3 class="text-2xl font-semibold text-gray-900 dark:text-white mb-2">{{ __('app.no_orders_found') }}</h3>
                    ...
                    @else
                        <h3 class="text-2xl font-semibold text-gray-900 dark:text-white mb-2">{{ __('app.no_orders_yet') }}</h3>
                        <p class="text-gray-500 dark:text-gray-400 mb-8 max-w-md mx-auto">
                            {{ __('app.start_creating_orders') }}
                        </p>
                        <a href="{{ route('orders.create') }}" 
    ...
    @if($orderToDelete)
        <div class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50" 
             ...
             wire:key="delete-modal-{{ $orderToDelete }}">
            <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white dark:bg-gray-800" 
                ...
                <div class="mt-3 text-center">
                    ...
                    <h3 class="text-lg leading-6 font-medium text-gray-900 dark:text-white mt-4">{{ __('app.delete_order_confirm') }}</h3>
                    <div class="mt-2 px-7 py-3">
                        <p class="text-sm text-gray-500 dark:text-gray-400">
                            Are you sure you want to delete order #{{ $orderToDelete }}? This action cannot be undone and will permanently remove the order and all its items.
                        </p>
                    </div>
                    <div class="items-center px-4 py-3">
                        <div class="flex space-x-3">
                            <button 
                                wire:click="cancelDelete"
                                type="button"
                                class="flex-1 px-4 py-2 bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-200 text-base font-medium rounded-md shadow-sm hover:bg-gray-200 dark:hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-gray-300 transition-colors duration-200"
                            >
                                Cancel
                            </button>
                            <button 
                                ...
                                class="flex-1 px-4 py-2 bg-red-600 text-white text-base font-medium rounded-md shadow-sm hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500 transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
                            >
                                <span wire:loading.remove wire:target="deleteOrder">{{ __('app.delete') }}</span>
                                <span wire:loading wire:target="deleteOrder" class="flex items-center justify-center">
